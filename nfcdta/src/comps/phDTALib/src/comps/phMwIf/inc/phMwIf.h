...
typedef struct phMwIf_sDiscParams
{
    ...
    BOOLEAN blAnalogMode;/**< Analog Flag to Enable Poll[A,B,F(212&424)] & Listen[F-P2P,
                              CE-UICC Type A&B]*/
    /*RFU*/
}phMwIf_sDiscParams_t;

typedef struct phMwIf_sDiscParams phMwIf_sRfTechs_t;
 ...
 * \param[in] eDevType            Dev Type specifies whether it is UICC/ESE. HBE is ignored.
 * \param[in] sRoutingTech        Specifies Listen A/B/F for technology based routing for specified device
...
/**
 * \ingroup grp_mwif_lib
 * \brief Disconnect client to server
 *
 * This function register LLCP client
 *
 * \param[in] mwIfHandle          Middleware Interface Handle provided during init
 * \param[in] pvConnHandle        Client Connection Handle
 *
 * \retval #MWIFSTATUS_SUCCESS    MWIF LIB LLCP client registered successfully
 * \retval #MWIFSTATUS_FAILED     MWIF LIB failed to register LLCP client
 *
 */
MWIF_LIB_EXTEND MWIFSTATUS phMwIf_LlcpClientDisconnect( void*     pvMwIfHandle,
                                                        void*     pvConnHandle);
/**
 * \ingroup grp_mwif_lib
 * \brief Connect client to server

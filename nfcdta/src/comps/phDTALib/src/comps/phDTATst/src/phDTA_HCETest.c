...
DTASTATUS phDtaLibi_HceOperations()
{
    ...
    do {
        dwOsalStatus = phOsal_QueuePull(dtaLibHdl->queueHdl, (void**)&psQueueData, 0);
          ...
          if (psQueueData->uEvtType.eDpEvtType == PHMWIF_DEACTIVATED_EVT)
          {
    ...
    if (dwOsalStatus != OSALSTATUS_SUCCESS)
     ...
     phOsal_LogDebugU32d((const uint8_t*)"DTALib>sQueueData.uEvtInfo.uDpEvtInfo.sData.pvDataBuf = ",sQueueData.uEvtInfo.uDpEvtInfo.sData.pvDataBuf);
